      ...
      mockDb.saveMechanics(
        mechanics.map((mechanic) => {
          ...
          const activeOtCount = orders.filter(
            (candidate) =>
              ['REGISTRADA', 'DIAGNOSTICADA', 'PRESUPUESTO_ENVIADO', 'APROBADO', 'EN_PROGRESO', 'EN_ESPERA_DE_REPUESTO'].includes(
                candidate.status
              ) &&
          ...
          const currentAssignment = orders.find(
            (candidate) =>
              ...
              ['APROBADO', 'EN_PROGRESO', 'EN_ESPERA_DE_REPUESTO'].includes(candidate.status)
          );
          return {
            ...mechanic,
            activeOtCount,
