...
import {
  ...
  useConsumeSparePart,
} from '../features/work-orders/api/useConsumeSparePart';
...
describe('useConsumeSparePart', () => {
  ...
  it('invalidates the dependent work order queries after a success', async () => {
    server.use(
      http.post(CONSUME_PATH, () =>
        HttpResponse.json({
          id: 'qp-1',
    ...
    const keys = invalidateSpy.mock.calls.map((call) => call[0] as { queryKey: readonly unknown[] }).map((c) =>
      c.queryKey[0],
    );
