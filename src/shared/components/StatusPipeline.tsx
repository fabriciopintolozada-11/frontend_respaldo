...
export interface StatusPipelineProps {
  ...
  interactive?: boolean;
  onSelectNextStatus?: (status: WorkOrderStatus) => void;
  className?: string;
}

export function StatusPipeline({ currentStatus, interactive = false, onSelectNextStatus, className = '' }: StatusPipelineProps) {
          ...
          return (
            <div key={step.status} className="relative z-10 flex flex-col items-center group">
              <button
                ...
                onClick={() => {
                  if (isClickable && onSelectNextStatus) onSelectNextStatus(step.status);
                }}
